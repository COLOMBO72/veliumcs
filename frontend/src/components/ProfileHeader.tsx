...
export default function ProfileHeader({ profile, bans, faceit, viewCount  }: Props) {
  const { t } = useLang();
  ...
  const created = profile.timecreated ? new Date(profile.timecreated * 1000).getFullYear() : '?';
  const lastOnline = profile.lastlogoff
    ? new Date(profile.lastlogoff * 1000).toLocaleDateString(locale)
    : '?';

  return (
    <div className="clip-tr2 animate-fadeup" style={{
      display: 'grid',
      ...
      <div style={{ position: 'relative', width: 100, height: 100, flexShrink: 0 }}>
        <img
          ...
          alt="avatar"
          loading="lazy"
          style={{ width: '100%', height: '100%', objectFit: 'cover', border: '2px solid var(--border2)', display: 'block' }}
        />
        {/* Online dot */}
        <div style={{
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
          ...
          {bans.VACBanned && (
            <span className="meta-tag vac">{t.tagVac}</span>
          )}
          {bans.NumberOfGameBans > 0 && (
            <span className="meta-tag gameban">{t.tagGameBan} ×{bans.NumberOfGameBans}</span>
          )}
...
function ExternalLink({ href, label, accent }: { href: string; label: string; accent?: string }) {
  return (
    <a
      href={href}
      target="_blank"
