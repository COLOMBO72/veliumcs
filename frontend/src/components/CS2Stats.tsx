...
interface Props { stats: CS2Stat[] }

const BAR_COLORS = ['', 'blue', 'green', 'teal', 'purple', 'blue', 'green', 'blue'];
...
export default function CS2Stats({ stats }: Props) {
  ...
  const roundWins   = getStat(stats, 'total_wins');             // round wins (NOT match wins)
  ...
  // Round win rate (different from match WR)
  const roundWR = roundsPlay > 0 ? Math.round(roundWins / roundsPlay * 100) : 0;
  ...
  const mapList = [
    { wKey: 'total_wins_map_de_dust2',    rKey: 'total_rounds_map_de_dust2',    name: 'Dust2' },
    { wKey: 'total_wins_map_de_inferno',  rKey: 'total_rounds_map_de_inferno',  name: 'Inferno' },
    { wKey: 'total_wins_map_de_nuke',     rKey: 'total_rounds_map_de_nuke',     name: 'Nuke' },
    { wKey: 'total_wins_map_de_train',    rKey: 'total_rounds_map_de_train',    name: 'Train' },
    { wKey: 'total_wins_map_de_vertigo',  rKey: 'total_rounds_map_de_vertigo',  name: 'Vertigo' },
    { wKey: 'total_wins_map_de_cbble',    rKey: 'total_rounds_map_de_cbble',    name: 'Cobblestone' },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 2, marginTop: 2 }}>
        {weapons.length > 0 && (
          <BarCard title={t.barWeapons} items={weapons.map(w => ({ name: w.name, val: w.val }))} max={mxW} />
        )}
        {mapList.length > 0 && (
...
function BarCard({
  ...
  return (
    <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', padding: 24 }}>
      <div style={{
        fontFamily: 'Rajdhani, sans-serif', fontSize: 13, fontWeight: 700,
        letterSpacing: '0.14em', textTransform: 'uppercase',
        color: 'var(--text2)', marginBottom: 18,
      }}>
        {title}
      </div>
      {items.map((item, i) => (
        <div key={item.name} style={{
          display: 'grid', gridTemplateColumns: '120px 1fr 56px',
          alignItems: 'center', gap: 12, marginBottom: 12,
        }}>
          <div style={{
            fontSize: 13, color: 'var(--text2)', textAlign: 'right',
...
function MapBarCard({
  ...
  return (
    <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', padding: 24 }}>
      ...
      {maps.map((m, i) => {
        const mapWR = m.rounds > 0 ? Math.round(m.wins / (m.rounds / 16) * 100) : 0;
        const clamp = Math.min(mapWR, 99);
        return (
